...
tesellation_energy_double<-function(xt,yt){
  ...
  tesener<-sum((areas-1)^2+(gamma_ad/2)*(perims^2)+lambda_ad*perims)
  
  tesel2<-deldir(xt*(Radius2/Radius),yt,rw=rec2)
  tilest2<-tile.list(tesel2)[(n+1):(2*n)]
  perims2<-(tilePerim(tilest2)$perimeters)/sqrt(A0)
  areas2<-sapply(tilest2,function(x){x$area})/A0
  tesener2<-sum((areas2-1)^2+(gamma_ad/2)*(perims2^2)+lambda_ad*perims2)
...
rec <- c(xmin,xmin+3*cyl_width,ymin,ymax)
rec2 <- c(xmin,xmin+3*cyl_width2,ymin,ymax)

energytesel <- tesellation_energy_double(points$x,points$y)
...
for (j in 1:steps) {
  for(l in 1:n) {
    ...
    energytesel2 <- tesellation_energy_double(points2$x,points2$y)
